...
public class CollaborationService {
 ...
 @Transactional public void setAdministrator(Long id,boolean administrator){requireSuperAdmin();if(mapper.registrationExists(id)==0)throw new BusinessException("用户不存在或不能变更该身份");mapper.removeBusinessAdmin(id);if(mapper.bindExternalRole(id)!=1)throw new BusinessException("普通员工角色未配置，请联系管理员");if(administrator&&mapper.addBusinessAdmin(id)!=1)throw new BusinessException("采购专员角色未配置，请联系超级管理员");}
 ...
 public void feedback(Long id,DocumentFeedbackRequest r){AuthUserRow u=current();requireAdmin();String status=r.getStatus();if(status==null||!Set.of("APPROVED","REVISION_REQUIRED").contains(status))throw new BusinessException("审核结果无效");String note=Optional.ofNullable(r.getFeedback()).orElse("").trim();if("REVISION_REQUIRED".equals(status)&&note.isBlank())throw new BusinessException("退回修改时必须填写原因");if(mapper.reviewDocument(id,status,note,u.getId(),u.getNickname())==0)throw new BusinessException("该PPT已由其他采购专员处理，请刷新列表");}
 private AuthUserRow current(){AuthUserRow u=auth.selectUserByUsername(username());if(u==null||!Integer.valueOf(1).equals(u.getStatus()))throw new BusinessException(401,"登录已过期");return u;}
