...
public class InitiationAttachmentService {
  private final InitiationMapper mapper; private final ProjectAccessService access; private final Path root;
  public InitiationAttachmentService(InitiationMapper mapper,ProjectAccessService access,@Value("${scaffold.initiation.storage-path:./data/initiation}") String path){this.mapper=mapper;this.access=access;this.root=Path.of(path).toAbsolutePath().normalize().resolve("attachments");}
  public List<Map<String,Object>> list(Long projectId){access.initiation(projectId);return mapper.attachments(projectId);}
  public IdResponse upload(Long projectId,String type,MultipartFile file){access.initiation(projectId);
  ...
  public Map.Entry<String,byte[]> download(Long id){access.attachment(id);Map<String,Object> a=mapper.attachment(id);if(a==null)throw new BusinessException(404,"附件不存在");try{return Map.entry(String.valueOf(a.get("originalFilename")),Files.readAllBytes(Path.of(String.valueOf(a.get("storagePath")))));}catch(IOException e){throw new BusinessException(404,"附件文件不存在");}}
  public void delete(Long id){access.attachment(id);Map<String,Object>a=mapper.attachment(id);if(a==null)return;mapper.deleteAttachment(id);try{Files.deleteIfExists(Path.of(String.valueOf(a.get("storagePath"))));}catch(IOException ignored){}}
